...
class UserListUpdateWorker (
    ...
    private val backendRepository: BackendRepository by inject()
    ...
    override suspend fun doWork(): Result {
        ...
        supervisorScope {
            ...
            val subscriptions = async(Dispatchers.IO) {
                listRepository.selectSubscriptions(user.id).getOrNull()
            }

            val userCreated  = async(Dispatchers.IO) {
                listRepository.selectListsByUserId(user.id).getOrNull()
            }

            val username = async(Dispatchers.IO) {
                backendRepository.getUser(user.id).getOrNull()?.username
            }
